...
- Live site (26 September 2026, `db/build.json`): 13,622 entries, 3,261 of them scores, a cover for every song and 340 merged songs' old addresses sent on; built by Vercel from `main` with the score import and the duplicate step. Its base catalogue is built fresh from the datasets, so it differs slightly from the older local database (45 songs play another Lakh file of the same song, 9 more entries); ids are pinned in `catalogue-ids.tsv` either way. `STRUDELIFY_URL=https://www.arthurenard.me/strudelify node tools/ui-check.mjs` runs the browser checks against it (all passed on 26 September 2026).
