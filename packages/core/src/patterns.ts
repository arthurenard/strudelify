...
function renderBars(t: Track, song: Song, firstBar: number, nBars: number, token: (n: NoteEvent) => string) {
  ...
  const notes = t.notes.filter(n => n.start >= from && n.start < end).map(n => ({ ...n,
    start: Math.min(end - 1e-7, Math.max(from, tidy(n.start - from, song.meta.bpm) + from)),
    duration: Math.max(1e-7, tidy(n.duration, song.meta.bpm)),
  }));
  ...
  const bars: Bar[] = buckets.map((ns, b) => {
    ...
    if (t.role !== 'drums' && chordGroups.every(g => g.length > 1)) {
    ...
    const legato = starts.every((s, i) => groups.get(s)!.every(n => Math.abs(Math.min(n.duration, end - n.start) / length - spans[i] / units) < 1e-6));
    const explicit = !legato && !uniform;
    ...
    const suffix = (mapped ? `.as([${mapping.map(k => "'" + k + "'").join(', ')}])` : '') + (legato ? '.legato(1)' : uniform ? `.duration(${durations[0]})` : '') + voicing;
...
export function compilePatterns(song: Song, opts: Required<CompileOptions>): string[] {
  ...
  const lines = ['// Live coding: edit a riff once to change every repeat.', '// Timing cleaned within 15 ms; choose Source detail for the unrounded events.', ''];
  ...
  const emit = (t: Track, base: string, token: (n: NoteEvent) => string, sound: string) => {
    ...
    const { bars, setters } = renderBars(t, song, range.firstBar, range.nBars, token);
    if (bars.every(b => b.body === '~')) return;
    const { definitions, arrangement } = arrangePhrases(bars, name, used);
    names.push(name);
    lines.push(`// ${partName(t.name) || base}: reusable phrases`, ...definitions, `// ${t.role} · ${base}`, `const ${name} = ${arrangement}`, `  ${setters}${sound}`, '');
  };
  ...
  lines.push('// Mix: mute a part here, or change its sound and effects above.', names.length ? `stack(\n  ${names.join(',\n  ')}\n)` : 'silence');
  return lines;
}
