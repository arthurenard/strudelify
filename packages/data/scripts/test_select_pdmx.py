...
def row(**overrides):
    return dict(license_conflict='False', is_draft='False', has_paywall='False', **{'subset:all_valid': 'True', 'song_length.seconds': '120'},
        is_best_unique_arrangement='True', has_lyrics='False', rating='4.8', n_ratings='10', n_favorites='0', n_tracks='3', n_notes='300',
...
class SelectionTests(unittest.TestCase):
    def test_quality_filters(self):
        ...
        for key, value in [('license_conflict', 'True'), ('is_draft', 'True'), ('has_paywall', 'True'), ('has_lyrics', 'True'), ('rating', '3.9'), ('n_ratings', '2'), ('n_tracks', '1'), ('n_notes', '99'), ('composer_name', 'unknown'), ('composer_name', '\u00aa'), ('song_name', '\u00aa -')]:
            with self.subTest(key=key): self.assertFalse(eligible({**valid, key: value}))

    def test_rated_or_liked(self):
        valid = row()
        self.assertTrue(eligible({**valid, 'rating': '4.0', 'n_ratings': '3'}))
        # Fewer than three ratings: ten favourites make a score good enough, unless a rating says otherwise.
        self.assertTrue(eligible({**valid, 'rating': '0', 'n_ratings': '0', 'n_favorites': '10'}))
        self.assertTrue(eligible({**valid, 'rating': '4.5', 'n_ratings': '1', 'n_favorites': '12'}))
        self.assertFalse(eligible({**valid, 'rating': '0', 'n_ratings': '0', 'n_favorites': '9'}))
        self.assertFalse(eligible({**valid, 'rating': '3', 'n_ratings': '2', 'n_favorites': '50'}))
