...
The WAV files are unmodified. `provenance.json` records each source URL and SHA-256. `samples.json` keeps the Strudel default sample indices; only the reviewed hits use local URLs. Other variants continue to use the upstream bank. Regenerate with `node tools/vendor-acoustic-samples.mjs` after building core.
